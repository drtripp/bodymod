...
| i18n groundwork | `frontend/src/lib/i18n.js`, `frontend/src/App.jsx`, `frontend/src/components/SiteHeader.jsx`, `frontend/src/components/OnboardingPanel.jsx`, `frontend/src/components/MeasurementForm.jsx`, `frontend/tests/i18n.node.mjs`, and Playwright shell coverage implement persisted locale selection, a message catalog, fallback/interpolation helpers, English/Spanish top-level shell/header/tab strings, first-run onboarding copy, measurement categories/field labels/help, unit controls, and measurement-guide chrome. | Expanded groundwork done; full account/diet/strategy/report translation pass pending |
...
| Tests simulating frontend users | `frontend/tests/app.spec.js` and `frontend/tests/mobile.spec.js` cover desktop and phone workflows with mocked backend responses, persisted theme switching, Spanish shell/onboarding/measurement-form localization, keyboard landmarks, live statuses, chart descriptions, local Pro waitlist capture, magic-link identity request/verify/clear, data explainer generation/boundary behavior, no-backend behavior, target comparison, match-priority switching, front/side silhouette view switching and line-art rendering, population chart mode/axis controls, snapshots, snapshot history charts, first-snapshot notification permission storage, encoded and read-only server share flows, historical weight import, limb-symmetry logging, cycle-context logging/deletion, account-free and signed-in JSON export downloads, encrypted backup restore, native backup unavailable messaging in web mode, encrypted sync-vault create/push/stale-conflict/merge/pull/revoke plus personal data API token issue/read/revoke, home-widget snapshot refresh, and health-sync preview preparation through the account UI, goal target-distance framing, life-event goal pausing, procedure tracker logging/backup/share/report persistence, maintenance drift alerts, Diet USDA/Open Food Facts search, barcode/logging/import, expanded micronutrient totals, public landing, methodology, measurement-guide, and legal page availability, 10 persona account walkthroughs including Jules' procedure log, strategy corpus age gating, high-risk corpus acknowledgment, linked strategy case-log rendering/import, and strategy corpus use. Node helper suites cover parser, crypto, chart, projection, real-world silhouette QA profiles, comparison variants, accessibility contrast, notification permission/reminder behavior, procedure normalization/healing/case-log behavior, share-dashboard payload privacy/local state, encrypted sync blob privacy/local credential state, personal data API request privacy/bearer behavior, magic-link account request/session privacy, data explainer privacy/citation/boundary behavior, health-sync batch privacy/metadata behavior, copy/tone guardrails, home-widget payload privacy/storage behavior, native encrypted-backup file metadata/filesystem behavior, i18n catalog parity/fallback behavior, local backup merge, entitlement non-gating, theme persistence, corpus safety gating, case-log bundle parsing, measurement-guide routing/static page coverage, limb-symmetry calculations, cycle-context calculations, goal pause rules, local JSON export shape, and calculation behavior. | Done |
...
- backend pytest passed `74` tests
...
- Node magic-link account validation passed `3` tests
