...
| Native push token and haptics pass | `frontend/src/lib/notifications.js`, `frontend/src/lib/haptics.js`, `frontend/src/components/AccountGoalPanel.jsx`, `frontend/package.json`, `frontend/package-lock.json`, `frontend/native-readme.md`, `backend/app/models.py`, `backend/app/repositories.py`, `backend/app/main.py`, `backend/tests/test_api.py`, `backend/tests/test_repositories.py`, `frontend/tests/notifications.node.mjs`, `frontend/tests/haptics.node.mjs`, and `verify.ps1` add `@capacitor/push-notifications` and `@capacitor/haptics`, register native push tokens only in Capacitor runtimes, persist backend token rows keyed by hash with timestamp-only reminder schedules, revoke by token hash, reject measurement payloads, and fire best-effort native success haptics after account check-in saves. | Token scaffold/haptics done; native provider send job, safe-area/status-bar/splash/icon polish pending |
