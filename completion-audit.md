...
| Remote web-push subscription scaffold | `frontend/src/lib/notifications.js`, `frontend/src/components/AccountGoalPanel.jsx`, `backend/app/models.py`, `backend/app/repositories.py`, `backend/app/main.py`, and notification/backend tests implement VAPID config discovery, explicit browser subscription/unsubscribe controls, strict subscription envelopes, and SQLite storage without accepting measurement payloads. | Subscription scaffold done; scheduled delivery worker and native push pending |
...
- Node notification validation passed `7` tests
