...
| i18n groundwork | `frontend/src/lib/i18n.js`, `frontend/src/lib/progressReport.js`, `frontend/src/App.jsx`, `frontend/src/components/SiteHeader.jsx`, `frontend/src/components/OnboardingPanel.jsx`, `frontend/src/components/MeasurementForm.jsx`, `frontend/src/components/DietDashboard.jsx`, `frontend/src/components/StrategyCorpus.jsx`, `frontend/src/components/AccountGoalPanel.jsx`, `frontend/src/components/FaceMeasurementPanel.jsx`, `frontend/tests/i18n.node.mjs`, `frontend/tests/progressReport.node.mjs`, and Playwright shell/diet/strategy/account/report coverage implement persisted locale selection, a message catalog, fallback/interpolation helpers, English/Spanish top-level shell/header/tab strings, first-run onboarding copy, measurement categories/field labels/help, unit controls, measurement-guide chrome, Body result/comparison/snapshot/population chrome, Diet dashboard chrome/status strings, Strategy explorer chrome/safety copy, account shell/profile switcher/magic-link identity/share-dashboard/home-widget/health-sync/live-update/encrypted-backup/native-backup/encrypted-sync/auto-sync/personal-data-api/entitlement/referral/data-explainer/face-measurement/check-in-tracking/goal-protocol-planning/procedure/bloodwork/workout/photo-log/export/report controls, and printable progress-report headings, empty states, and generated body rows. | Expanded groundwork done; source, seed, and user-authored content localization pending |
...
| Curation review packet scaffold | `backend/app/data/curation_review.seed.json`, `backend/app/data/curation_review.py`, `backend/app/models.py`, `backend/app/main.py`, `backend/scripts/validate_curation.py`, `frontend/src/lib/curationReview.js`, `frontend/src/components/AccountGoalPanel.jsx`, `frontend/tests/curationReview.node.mjs`, `frontend/tests/app.spec.js`, `feature-backlog.md`, and `manual-work-queue.md` add metadata-only review packets for strategy corpus, target profiles, reference data, FoodData Central, attractiveness evidence, procedure taxonomy, and bloodwork marker/range inputs. It is served at `/api/curation-review-packets`, shown in the account panel, and validates linked launch gates, seed files, reviewer questions, acceptance criteria, and docs. | Scaffold done; human source/content/clinical review remains blocking |
...
| Tests simulating frontend users | `frontend/tests/app.spec.js` and `frontend/tests/mobile.spec.js` cover desktop and phone workflows with mocked backend responses, persisted theme switching, Spanish shell/onboarding/measurement-form/Body result-comparison-population/Diet/Strategy/account localization, localized account/progress-report downloads, keyboard landmarks, live statuses, chart descriptions, local Pro waitlist capture, magic-link identity request/verify/clear plus email-link URL token loading/scrubbing, data explainer generation/boundary behavior, no-backend behavior, target comparison, match-priority switching, front/side silhouette view switching and line-art rendering, population chart mode/axis controls, snapshots, snapshot history charts, first-snapshot notification permission storage, encoded URL, expiring opaque snapshot, and read-only server share flows, historical weight import, limb-symmetry logging, cycle-context logging/deletion, account-free and signed-in JSON export downloads, encrypted backup restore, native backup unavailable messaging in web mode, encrypted sync-vault create/push/stale-conflict/merge/auto-sync-preview/pull/revoke plus personal data API token issue/read/revoke, manual side-profile face log export/report coverage, home-widget snapshot refresh, health-sync preview preparation, live-update manifest checks, launch-readiness gate checks, and case-log moderation submission through the account UI, goal target-distance framing, life-event goal pausing, procedure tracker logging/backup/share/report persistence, maintenance drift alerts, Diet USDA/Open Food Facts search, barcode/logging/import, expanded micronutrient totals, public landing, methodology, measurement-guide, and legal page availability, 10 persona account walkthroughs including Jules' procedure log, strategy corpus age gating, high-risk corpus acknowledgment, linked strategy case-log rendering/import, and strategy corpus use. Node helper suites cover parser, crypto, chart, projection, real-world silhouette QA profiles, comparison variants, accessibility contrast, notification permission/reminder behavior, face landmark and manual side-profile log privacy, procedure normalization/healing/case-log behavior, case-log submission privacy envelopes, share-dashboard payload privacy/local state, share-snapshot payload privacy/URL behavior, encrypted sync blob privacy/local credential state, encrypted sync auto-run readiness/storage privacy, personal data API request privacy/bearer behavior, magic-link account request/session/privacy and URL-token parsing, data explainer privacy/citation/boundary behavior, health-sync batch privacy/metadata behavior, live-update manifest privacy/status behavior, launch-readiness gate normalization, copy/tone guardrails, home-widget payload privacy/storage behavior, native encrypted-backup file metadata/filesystem behavior, i18n catalog parity/fallback behavior, progress-report output/localization, local backup merge, entitlement non-gating, theme persistence, corpus safety gating, case-log bundle parsing, measurement-guide routing/static page coverage, limb-symmetry calculations, cycle-context calculations, goal pause rules, local JSON export shape, and calculation behavior. | Done |
...
- backend pytest passed `105` tests
- curation JSON validation passed for target/corpus seeds, templates, the ANSUR mapping file, `9` launch-readiness gates, `10` provider decisions, `8` native release items, `7` curation review packets, face-model candidates, and the corpus moderation policy seed
...
- Node share-snapshot validation passed `2` tests
...
- Node i18n validation passed `5` tests, including account identity,
  share-dashboard, widget, health-sync, live-update, encrypted-backup, and
  native-backup, encrypted-sync, auto-sync, personal-data API, entitlement,
  referral, data-explainer, face-measurement, goal/protocol planning, and Body
  result/comparison/snapshot/population interpolation
- Node progress report validation passed `1` test, including populated Spanish
...
- Node curation-review validation passed `3` tests
...
- desktop/mobile screenshot review passed; targeted side-view, match-priority,
  ...
  entitlement, corpus age-gate, high-risk acknowledgment, strategy
  case-log detail, and real-server account case-log submission screenshots
  ...
  copy, corpus safety controls, linked case-log detail layout, and queued
  case-log submission card/status are readable on desktop and mobile; targeted
  real-server desktop/mobile screenshots confirmed the curation review packet
  section is readable without overlap
...
into the validated `/api/launch-readiness` gate seed and
`/api/curation-review-packets` packet seed. User protocol case logs can now
enter a review-only `/api/case-log-submissions` queue, and the app exposes a
