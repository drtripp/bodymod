...
test("translates known keys, falls back to English, and interpolates values", () => {
  ...
  assert.equal(
    translate("es", "account.provider.status.loaded", { decisions: 10, blocking: 10 }),
    "10 decision(es) de proveedor cargada(s); 10 aun bloquean completar."
  );
  assert.equal(
    translate("es", "account.nativeRelease.status.loaded", { items: 8, blocking: 8 }),
